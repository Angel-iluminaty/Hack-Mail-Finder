...
def update():
    ...
    try:
        ...
        print(f"{space}{b}[{w}!{b}]{w} Error! No se pudo iniciar MailFinder, verifique si Python está instalado correctamente...")

def restart():
    print("{space}{b}[{w}01{b}]{w} Error! Olvidaste escribir algo...")
    time.sleep(2)
    cls()
...
def banner():
    ...
    print(f'                  {w}.:.:;..{y}Hacker MailFinder v'+version+f' Creador: Angel Del Villar{w}..;:.:.')
    print(f'\n{des_space}{b}>> {w}Para encontrar el correo que necesita, escriba su apellido y \n{des_space}{b}>> {w}nombre de diferentes maneras. Por ejemplo: vuiko, vuikoo, vu\n{des_space}{b}>> {w}It Todavía lleva mucho tiempo encontrar su e-mail.\n')

def selecttype():
    #print(f"{space}{b}[{w}1{b}]{w} Search e-mail from database")
    print(f"{space}{b}[{w}1{b}]{w} Verificar dominio de e-mail en lista negra")
    print(f"{space}{b}[{w}2{b}]{w} Verifique el nombre de usuario en los dominios de Email")
    print(f"{space} {w}|")
    print(f"{space}{b}[{w}3{b}]{w} Buscar información de e-mail, hunter.io")
    print(f"{space}{b}[{w}4{b}]{w} Buscar e-mail a través del nombre completo")
    print(f"{space}{b}[{w}5{b}]{w} Actualizar Buscador de e-mail\n")
...
def checkdomain():
    cls()
    banner()
    domain = str(input(f"{space}{b}[{w}?{b}]{w}  dominio (ex. gmail.com):{b} ").lower())
    get = requests.get(validator_url+"domain/verify?"+str(domain)).text
    print(" ")
    if "ok::good domain" in get:
        print(f"{space}{b}[{w}+{b}]{w} Dominio verificado : Yes")
        print(f"{space}{b}[{w}+{b}]{w} Dominio de phishing : No")
        print(f"{space}{b}[{w}+{b}]{w} Dominio falso     : No")
    else:
        print(f"{space}{b}[{w}+{b}]{w} Dominio verificado : No")
        print(f"{space}{b}[{w}+{b}]{w} Dominio de phishing : Unknown")
        print(f"{space}{b}[{w}+{b}]{w} Dominio falso     : Unknown")
    time.sleep(1)

def emailinfo():
    h_api = random.choice(h_apis)
    auto()
    email = str(input(f"{space}{b}[{w}?{b}]{w} Escribe un e-mail para obtener información.:{b} ").lower())
    auto()
    try:
        ...
        auto()
        print(f"{space}{b}[{w}!{b}]{w} Oh no, parece que las claves API gratuitas se han agotado.")
        print(f"{space}{b}[{w}!{b}]{w} Cree su clave API en el sitio web hunter.io")
        h_api = str(input(f"{space}{b}[{w}?{b}]{w} Escribe tu clave API de Hunter.io:{b} ").lower())
        get = requests.get("https://api.hunter.io/v2/email-verifier?email="+email+"&api_key="+h_api).json()
        data = get['data']
        one = str(data['status'])
        two = str(data['result'])
...
def validator():
    auto()
    user = str(input(f"{space}{b}[{w}?{b}]{w} nombre de usuario para validar el e-mail:{b} ").lower())
    auto()
    print(w+lines)
    data = [
        "gmail.com",
...
def mailfinder():
    ...
    cls()
    banner()
    name = str(input(f"{space}{b}[{w}?{b}]{w} nombre de la víctima:{b} ").lower())
    surname = str(input(f"{space}{b}[{w}?{b}]{w} apellido de la víctima:{b} ").lower())
    time.sleep(1.5)
    cls()
    banner()
    ...
    data = []
    print(f"""{space}{b}[{w}?{b}]{w} Para búsqueda automática, escriba 'auto'""")
    type = str(input(f"{space}{b}[{w}?{b}]{w} dominio (ex. gmail.com):{b} ").lower())
    if type == "auto":
